...
func printUsage(f *os.File, kind usageType) {
	const template = `
USAGE:
{{.Tab1}}{{.AppName}} [-c count] [-t timeout] [-tcponly] [-json] [-csv] [-csv-no-header] [-ca=<file>]
...
{{.Tab1}}-t timeout
{{.Tab2}}Number of seconds dial connections timeout.
...
{{.Tab1}}-csv
{{.Tab2}}Format the result in CSV format and print to standard output. Reported
{{.Tab2}}times are understood in seconds.

{{.Tab1}}-csv-no-header
{{.Tab2}}Format the result in CSV format without header and print to standard output. Reported
{{.Tab2}}times are understood in seconds.
